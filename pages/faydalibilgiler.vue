...
<div v-show="sideMenu && articleData" class="sidenav">
    ...
    <ul class="p-y-4 border-t-2 border-white  overflow-y-auto">
      <li v-for="(item, index) in articleData":key="item.id" v-text="item.title" class="hover:bg-gray-200 transition-colors duration-200 p-2 whitespace-pre-wrap font-bold" :class="{'!bg-red-500' : currentPage==item.tag}" @click="pageChanged(item)"></li>
          
      </ul>
</div>
          </Transition>

<div v-show="articleData" class="transition-colors duration-300 fixed size-14 cursor-pointer mt-1 rounded-lg border-2 border-white bg-red-500  ml-8 z-50 hover:bg-orange-500 active:bg-red-700" @click="sideMenu = !sideMenu;">
...
<div :key="currentPage" class="article min-[880px]:w-[80%] overflow-x-auto  min-[880px]:overflow-x-visible min-[880px]:flex min-[880px]:justify-center" v-html="htmlContent"></div>
        </Transition>
</div>
