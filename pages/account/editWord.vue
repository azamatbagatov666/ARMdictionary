  ...
  if (confirm("Sonuç belirlediğiniz şekilde düzenlenecektir, emin misiniz?")) {
    if (
      ...
      for (const key in selectedIndex.value) {
        if (selectedIndex.value[key] === "" && selectedBackup.value[key] === (null || "")) {
      ...
      saveChanges(selectedIndex.value)
      .then((response) => {
        if (response.ok) {
...
<template>
  ...
  <div v-text="'Sonuç Düzenle'" class="bg-red-900 text-5xl text-center w-[500px] border-2 py-3 mx-auto inline-block border-black rounded-lg dark:border-white"></div>
</div> 
  ...
  <div class="mb-12">
    ...
    <div
      v-if="selectedIndex"
      class="text-3xl mb-[-20px] text-center mt-1"
      v-text="'Sonucun Şu Anki Hali'"
    ></div>

    <div v-for="(item, index) in responseData" :key="item.worD_ID">
      <table
    ...
    <div v-if="selectedIndex">
      <div
        class="text-3xl text-center mt-[-20px] mb-4"
      ...
      <div class="grid grid-cols-4 text-lg gap-y-2">
        ...
        <div class="flex items-center col-span-3">
          <label class="w-52 ml-3"
            >Kelimenin Okunuşu:<span style="color: red">*</span></label
          >
          <ElementComponentsCustomInput
            class="w-52 border border-black"
  ...
  <ElementComponentsCustomButton
    @click="updateTheWord"
    v-if="selectedIndex"
    class="block mx-auto mt-[-20px] mb-4"
    text="Sonucu Güncelle"
  />

  <div class="mx-auto" v-if="arananData">
    <div class="text-3xl">Bu sonucu veren sözcükler:</div>
    <ul class="list-disc text-lg pl-10">
      <li
