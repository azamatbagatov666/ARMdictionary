...
const deleteTheWords = async () => {
    if (confirm('Seçtiğiniz sözcükler bu sonuçtan silinecektir, emin misiniz?')) {
        ...
        try {
            const response = await fetch("https://localhost:7109/DeleteAndMoveToTrash", {
                method: "POST",
                headers: {
        ...
        window.alert("Herhangi bir seçim yapmadınız. Önce silinmesini istediğiniz sözükleri işaretleyin.");
        }
}
    ...
    <div class="mb-12">
        ...
        <table class="border border-solid border-gray-300 text-lg p-8 m-10 mx-auto w-1/2" v-for="item in responseData"
            ...
            <tr class="h-10">
                <td>
                    <label class="ml-2">
                        <input type="radio" name="wordSelection" @change="getAranan()" v-model="selectedRadio"
                            :value="item.id" />
        ...
        <div class="text-center mb-4">
            <ul class="list-none text-lg text-left inline-block">
                ...
                <li v-for="(arananlar, index) in arananData" :key="index">
                    <label class="mx-auto" :class="{ 'line-through': selectedListWord.includes(arananlar) }">
                        <input type="checkbox" v-model="selectedListWord" :value="arananlar" class="mr-2" />
        ...
        <button class="block mx-auto" @click="deleteTheWords">
            Seçilenleri Sil
        </button>
    </div>
</template>
