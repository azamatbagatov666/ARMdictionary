...
const consumedLetters = computed(() => {
  return (allGuesses.value.map((g) => g.word)).flatMap((w) => w.split(""))
    .filter((l, i, arr) => {
      return (
        arr.indexOf(l) === i &&
        !answerArray.value.includes(l)
      );
    });
...
<template>
  ...
  <div v-if="answer.length > 0">
        ...
        <div v-for="i in 6" >
          <div class="flex justify-center" :class="{ shake: invalidAnswer && i === currentGuessIndex }">
            <div
              ...
              :class="{
                '!border-black': i === currentGuessIndex && currentGuess[j - 1],
                '!text-black': i === currentGuessIndex,
    ...
    <Transition name="slide-fade">
        ...
        <div class="" >
      <ArmenianKeyboard class="!m-0 !border-t-0 !rounded-t-none !-mt-1 !mb-0 font-bold"
        @letter-pushed="push"
      ...
      >
          ...
          <div class="text-xl font-bold text-white my-2 min-h-7 text-center uppercase tracking-widest">{{ lose ? answer :'' }}</div>
